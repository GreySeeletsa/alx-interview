...
def build_solution(row, group):
    ...
    # Recursive case: if the solution is not complete, keep building it
        ...
        for col in range(n):
            ...
            # Check if any of the positions in curr solution group are
            # attacking position at index
            used_positions = map(lambda x: is_attacking(x[0], x[1]), matches)
            # If there is no attacking queen, add position to curr
            # solution group
            group.append(pos[a].copy())
            # Check if any of the positions in the current solution group are
            # attacking
            if not any(used_positions):
                # If not, move on to next row & keep building solution
                build_solution(row + 1, group)
            # Remove the queen from group before trying next column
            group.pop(len(group) - 1)


def get_solutions():
    """Generates all possible solutions to N queens problem by building
    ...
    """
    global pos, n
    # Map each of  N^2 positions to their respective row and column indices
    pos = list(map(lambda x: [x // n, x % n], range(n ** 2)))
    # Initialize a and group var
    a = 0
    group = []
    # Call build_solution func to build all solutions
    build_solution(a, group)


# Get input value of N from command line args
n = get_input()

# Call func to find all solutions of N-queens problem
get_solutions()

# Loop through all the solutions found
for solution in solutions:
    # Print each solution, repre as list of positions of queens on board
    print(solution)
